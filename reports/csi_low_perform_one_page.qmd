...
essa_filtered %>%
  ...
  mutate(
    ...
    color = case_match(color,
                       ...
                       "5" ~ "Blue",
                       .default = "Status only")) %>%
  select(student_group_long, indicator, color, currnumer_str, currdenom, currstatus, change) %>%
  arrange(student_group_long) %>% 
  flextable() %>% 
  ...
  align(align = c("left", "left", "left", "right", "right", "right", "right"), part = "all")
