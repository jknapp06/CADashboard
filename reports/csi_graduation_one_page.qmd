---
format: 
  pdf:
    mainfont: Ebrima
    fontsize: 10pt
    keep-tex: true
    margin-top: 0.5in
    margin-bottom: 0.75in
    margin-left: 1.2in
    margin-right: 1.2in
editor: visual
params:
  year: 2024
  school: "Maine Prairie High (Continuation)"
---

```{r setup, include = FALSE}

library(shiny)
library(tidyverse)
library(DT)
library(wesanderson)
library(fontawesome)
library(ggtext)
library(showtext)
library(flextable)

essa <- read_csv("data/dashboard_essa.csv") %>% 
  filter(countyname == "Solano",
         rtype == "S") %>% 
  mutate(studentGroup = as_factor(studentgroup),
         color = as_factor(color),
         studentGroupLong = fct_relevel(as_factor(student_group_long),
                                    "All students",
                                    "American Indian or Alaska Native",
                                    "Asian",
                                    "Black/African American",
                                    "Filipino",
                                    "Hispanic",
                                    "Pacific Islander",
                                    "White",
                                    "Multiple Races/Two or more",
                                    "Socioeconomically Disadvantaged",
                                    "Homeless Youth",
                                    "Students with Disabilities",
                                    "Foster Youth",
                                    "English Learner",
                                    "English Learners Only",
                                    "Long-Term English Learner",
                                    "RFEPs Only",
                                    "English Only",
                                    "Smarter Balanced Assessment",
                                    "CA Alternative Assessment"
                                    ),
                  indicator = case_match(indicator,
                                "ELA" ~ "ELA (DFS)",
                                "Math" ~ "Math (DFS)",
                                "college/career" ~ "college/career\n(status only)",
                                .default = indicator),
                  indicator = fct_relevel(as_factor(indicator), 
                                 "ELA (DFS)", "Math (DFS)", "ELPI", 
                                 "absenteeism", "graduation", 
                                 "suspension")) 

dashboard <- 
  read_csv("data/ca_dashboard.csv") %>% 
  filter(countyname == "Solano") %>% 
  mutate(statuslevel = as_factor(statuslevel),
         reportingyear = fct_relevel(as_factor(reportingyear), "2019"),
         indicator = case_match(indicator,
                                "ELA" ~ "ELA (DFS)",
                                "Math" ~ "Math (DFS)",
                                "college/career" ~ "college/career\n(status only)",
                                .default = indicator),
         indicator = fct_relevel(as_factor(indicator), 
                                 "ELA (DFS)", "Math (DFS)", "ELPI", 
                                 "absenteeism", "graduation", 
                                 "suspension"),
         studentgroup = as_factor(studentgroup),
         student_group_long = fct_relevel(as_factor(student_group_long),
                                    "All students",
                                    "American Indian or Alaska Native",
                                    "Asian",
                                    "Black/African American",
                                    "Filipino",
                                    "Hispanic",
                                    "Pacific Islander",
                                    "White",
                                    "Multiple Races/Two or more",
                                    "Socioeconomically Disadvantaged",
                                    "Homeless Youth",
                                    "Students with Disabilities",
                                    "Foster Youth",
                                    "English Learner",
                                    "English Learners Only",
                                    "Long-Term English Learner",
                                    "RFEPs Only",
                                    "English Only",
                                    "Smarter Balanced Assessment",
                                    "CA Alternative Assessment"
                                    ),
         charter_flag = if_else(is.na(charter_flag), "N", charter_flag),
         color = as_factor(color),
         lea = if_else(charter_flag == "Y", schoolname, districtname))


status_colors <- c(
  "lightgrey", "tomato", "orange",
  "yellow", "mediumseagreen", "dodgerblue", "darkgrey"
)

font_add('fa-solid', 'fonts/fa-solid-900.ttf')

# element_markdown uses <br> for newlines
html_wrap <- function(s, width = 17){
    str_wrap(s, width = width) %>% 
      str_replace_all("\n", "<br>")
  }

essa_filtered <- 
  essa %>% 
  filter(schoolname == params$school,
         reportingyear == params$year,
         student_group_long %in% c(
                                    "All students",
                                    "American Indian or Alaska Native",
                                    "Asian",
                                    "Black/African American",
                                    "Filipino",
                                    "Hispanic",
                                    "Pacific Islander",
                                    "White",
                                    "Multiple Races/Two or more",
                                    "Socioeconomically Disadvantaged",
                                    "Homeless Youth",
                                    "Students with Disabilities",
                                    "Foster Youth",
                                    "English Learner",
                                    "Long-Term English Learner")) %>% 
  mutate(student_group_wrapped = html_wrap(student_group_long, width = 17),
         change = if_else(is.na(change), "", paste(change)),
         change_arrow = if_else(indicator %in% c("ELA (DFS)", "Math (DFS)", "ELPI", 
                           "graduation", "college/career\n(status only)"),
                                case_match(changelevel,
                                   0 ~ "<span style='font-family:fa-solid;'>&#xf068;</span>",
                                   1 ~ "<span style='font-family:fa-solid;'>&#xf063;</span>",
                                   2 ~ "<span style='font-family:fa-solid;'>&#xf078;</span>",
                                   3 ~ "<span style='font-family:fa-solid;'>&#xf7a4;</span>",
                                   4 ~ "<span style='font-family:fa-solid;'>&#xf077;</span>",
                                   5 ~ "<span style='font-family:fa-solid;'>&#xf062;</span>",
                                   .default = "<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>"),
                                                                case_match(changelevel,
                                   0 ~ "<span style='font-family:fa-solid;'>&#xf068;</span>",
                                   5 ~ "<span style='font-family:fa-solid;'>&#xf063;</span>",
                                   4 ~ "<span style='font-family:fa-solid;'>&#xf078;</span>",
                                   3 ~ "<span style='font-family:fa-solid;'>&#xf7a4;</span>",
                                   2 ~ "<span style='font-family:fa-solid;'>&#xf077;</span>",
                                   1 ~ "<span style='font-family:fa-solid;'>&#xf062;</span>",
                                   .default = "<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>"))) %>% 
  arrange(student_group_long)

group_count <-
  essa_filtered %>% 
  select(student_group_long) %>% 
  unique() %>% 
  nrow()

group_order <- 
  essa_filtered %>% 
  select(student_group_long) %>% 
  unique() %>%  
  arrange(student_group_long) %>% 
  mutate(order = c(1:group_count))

```

![](www/SCOE_Logo.jpg){fig-align="right" width="127"}

# `r params$school` Comprehensive Support and Improvement Report `r paste0(params$year, "-", params$year + 1)`

## Comprehensive Support and Improvement (CSI) Eligibility

In accordance with Every Student Succeeds Act [(ESSA)](https://www.cde.ca.gov/re/es/), schools are eligible for CSI if they receive at least one color coded performance level on any state indicator and meet the criteria in one of the following categories:

1.  CSI--Low Graduation Rate For `r params$year`.

2.  CSI--Low Performing For `r params$year`.

The complete details for determining eligibility can be found in the technical guide for CSI released by the CA Department of Education (CDE). [Download the guide here.](https://www.cde.ca.gov/sp/sw/t1/csi.asp "Comprehensive Support and Improvement")

```{r get_years, include = FALSE}

csi_year_th <- 
  essa_filtered %>% 
  filter(studentgroup != "ALL") %>% 
  mutate(yearth = case_match(csi_years,
                             1 ~ "1st",
                             2 ~ "2nd",
                             3 ~ "3rd",
                             .default = paste0(csi_years, "th"))) %>% 
  pull(yearth) %>%
  nth(1)

```

ESSA support determinations under CSI were made for California following the publication of the 2023 Dashboard. These determinations marked the first year of California’s three-year identification cycle. Schools eligible for CSI (Low Graduation Rate/Low Performing) in 2023–24 will be evaluated annually to determine continued eligibility for CSI. Schools meeting the required exit criteria will be in the eligibility pool for Targeted Support and Improvement (TSI) determinations.

`r params$school` has qualified for CSI in the graduation category. **This is the school's `r csi_year_th` consecutive year qualifying for CSI.**

### CSI--Low Graduation Rate Formula

The sum of Graduates in `r paste0(params$year - 2, ", ", params$year - 1, ", and ", params$year)` .

**Divided by**

The sum of students in cohort in `r paste0(params$year - 2, ", ", params$year - 1, ", and ", params$year)` .

Table 1 provides the graduation rates of the past three year for `r params$school`.

```{r csi-table}
#| label: csi-eligible
#| echo: FALSE
#| tbl-cap: "Graduation Rates for the past three years"
#| cap-location: bottom

years <- as.character(seq(params$year - 2, params$year))

last_three_grad <- 
  dashboard %>%
  filter(schoolname == params$school,
         indicator == "graduation",
         student_group_long == "All students",
         reportingyear %in% years) 

last_three_grad %>% 
  mutate(
    currstatus = paste0(currstatus, "%"),
    currnumer_str = as.character(currnumer)) %>%
  select(reportingyear, currnumer_str, currdenom, currstatus, change) %>%
  arrange(desc(reportingyear)) %>% 
  flextable() %>% 
  line_spacing(space = 0.2,
               part = "body") %>% 
  set_header_labels(
    reportingyear = "Year",
    currnumer_str = "Numerator",
    currdenom = "Denominator",
    currstatus = "Current Status",
    change = "Change") %>%
  width(width = c(1.2, 1, 1, 1, 1)) %>%
  align(align = c("left", "right", "right", "right", "right"), part = "all")
  # set_table_properties(layout = "autofit")

grad_values <- 
  last_three_grad %>% 
  group_by(schoolname) %>% 
  summarise(graduates = sum(currnumer),
            all_students = sum(currdenom),
            grad_rate = round(graduates/all_students * 100), 1)

```

\* No numerator for this indicator.

The sum of students in the cohort for the past three years (`r paste0(params$year - 2, ", ", params$year - 1, ", and ", params$year)`) is `r grad_values$all_students[1]`, the sum of graduates for the past three years is `r grad_values$graduates[1]`. **The three-year combined four-and five-year graduation rate is `r paste0(grad_values$grad_rate[1],"%")`.** {{< pagebreak >}}

## California School Dashboard `r params$year`

The California School Dashboard (Dashboard) reflects California's accountability system with the reporting of Status (current year data), Change (the difference from prior year data), and Performance Levels (colors) for all state indicators except the Science indicator. Details below represent icon definitions included in the dashboard overview.

![](dashboard-cell-key-CDE-change%20CSI.png){fig-align="left"}

## `r params$school` Dashboard Overview `r params$year`

```{r dashboard-plot, echo = FALSE, warning=FALSE, fig.height = 4.8, fig.width = 8.4}

showtext_auto()

essa_filtered %>%
  left_join(group_order, by = join_by(student_group_long)) %>% 
  mutate(student_group_wrapped = fct_reorder(student_group_wrapped, 
                                              order)) %>% 
  ggplot() +
  geom_richtext(x = 0.5, y = 0.5,
            mapping = aes(label = if_else(indicator == "ELA (DFS)" |
                                          indicator == "Math (DFS)" |
                                          is.na(currstatus), 
                                          paste(currstatus, change_arrow, change),
                                          paste0(currstatus, "% ", change_arrow, " ", change)),
                          fill = color),
            show.legend = FALSE,
            text.color = "black",
            label.size = 0.8,
            size = 3.3) +
  scale_fill_manual(values = status_colors,
                    drop = FALSE,
                    na.value = "white") +
  facet_grid(student_group_wrapped ~ indicator, scales = "free", space = "free") +
  theme_minimal() +
  theme(
    text = element_text(size = 12),
    strip.text.x = element_text(size = 11),
    strip.text.y = element_markdown(size = 11, angle = 0, vjust = 0.7, hjust = 0),
    strip.clip = "off",
    plot.caption = element_text(size = 12),
    panel.spacing = unit(0, "pt")
  )

showtext_auto(FALSE)
```

Report prepared by Solano County Office of Education Assessment Research and Evaluation team. Email Dr. Tacey Rodgers trodgers\@solanocoe.net and Joseph Knapp jknapp\@solanocoe.net with any questions.
